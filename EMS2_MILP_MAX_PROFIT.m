...
soc_capacity = 150; % kWh soc_capacity 
...
%constraint part
epicons1 = Resolution*Buy_rate.*(Pchg - Pdchg) - u <=  Resolution*Buy_rate.*(PV-PL);
epicons2 = Resolution*Sell_rate.*(Pchg - Pdchg) - u <=  Resolution*Sell_rate.*(PV-PL);
chargecons = Pchg  <= xchg*cr;
dischargecons = Pdchg  <= xdchg*dr;
powercons = Pnet  == PV + Pdchg - PL - Pchg;
...
prob.Constraints.epicons1 = epicons1;
prob.Constraints.epicons2 = epicons2;
...
Pload = sol.Pchg + PL; % Load + Battery charge
...
stairs(vect,[Buy_rate,Sell_rate])
legend('Buy rate','Sell_rate')
%set(gca,'YLim',[0 6])
xlabel('Hour') 
title('TOU') 
ylabel('TOU (THB)')
...
nexttile;
stairs(vect,cumsum(revenue)); ylabel('Profit (THB)')
...
function [PV,PL,Buy_rate,Sell_rate] = getProfile(fs,h,start_date,end_date,TOU_CHOICE)
    %k = h*fs; 
    PV_scale_factor = 6; % scale up from 8kW to 4*8
