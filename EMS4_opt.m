function sol = EMS4_opt(PARAM,dataset_name,is_save,save_path) 
    ...
    %---------- PV ----------
    prob.Constraints.PVcon = optimconstr(k);
    prob.Constraints.PVcon(1:k) = PV(1:k) <= PARAM.PV(1:k);
    %--battery constraint
    ...
    prob.Constraints.powercons = Pnet  == PV + sum(Pdchg,2) - PARAM.Puload - Pac_lab - sum(Pchg,2) - Pac_student;
    ...
    if is_save == 1
        ...
        save(strcat(save_path,'/EMS4/',num2str(PARAM.battery.num_batt),'batt/',PARAM.TOU_CHOICE,'_',dataset_name,'.mat'),'-struct','sol')
