...
for i = 1:48
    ...
    networth_without_ems_thcurrent(i) = sum(GetExpense(sol_thcurrent.PARAM.PV-sol_thcurrent.PARAM.PL,...
                                        sol_thcurrent.PARAM.Buy_rate, ...
                                        sol_thcurrent.PARAM.Sell_rate, ...
                                        sol_thcurrent.PARAM.Resolution));  
    networth_without_ems_smart(i) = sum(GetExpense(sol_smart.PARAM.PV-sol_smart.PARAM.PL, ...
...
title('networth when TOU THcurrent is used (+ profit -expense)')
ylabel('networth(THB)')
...
title('histogram of networth gain by EMS2 when TOU is THcurrent')
xlabel('networth(THB)')
...
title('histogram of networth gain by EMS2 when TOU is smart1')
xlabel('networth(THB)')
