function sol = EMS5_opt(PARAM,dataset_name,is_save,save_path) 
    ...
    maxPnet1day = optimvar('maxPnet1day',PARAM.Horizon,'LowerBound',0,'UpperBound',inf);
    ...
    prob =      optimproblem('Objective',sum(maxPnet1day) + sum(s) );
    ...
    %-- RE100 constraint
    REcons =  optimconstr(k,2);
    for i = 0:(PARAM.Horizon - 1)
        REcons(h*i + 1:h*(i+1),1) = -Pnet(h*i + 1:h*(i+1)) <= u(h*i + 1:h*(i+1));
        REcons(h*i + 1:h*(i+1),2) = u(h*i + 1:h*(i+1)) <= maxPnet1day(i+1);
    end
    ...
    if is_save == 1
        ...
        save(strcat(save_path,'/EMS5/',num2str(PARAM.battery.num_batt),'batt/',PARAM.TOU_CHOICE,'_',dataset_name,'.mat'),'-struct','sol')
